 ...
 *  ✅ Railway-compatible: better-sqlite3, trust proxy, mobile-ready
 ********************************************************************/
...
// ✅ Trust Railway's reverse proxy (fixes login/session loop on HTTPS)
...
app.use(session({
    ...
    proxy: true, // ✅ Required for Railway HTTPS cookies
    cookie: { 
...
app.get('/login', (req, res) => { 
  res.render('login', { 
    ...
    success: req.query.registered === 'true' ? 'Account created successfully!' : null 
  }); 
});
...
app.post('/register', async (req, res) => {
  ...
  if (await sqlGet(`SELECT id FROM users WHERE username = ?`, [username])) {
    return res.render('register', { error: 'Username already taken.' });
  }
  const hashed = bcrypt.hashSync(password, 10);
  const result = await sqlRun(
    `INSERT INTO users (username, password, firstName, lastName, email, memberSince, address, city, state, zip, isAdmin, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
...
app.get('/transactions', requireLogin, async (req, res) => {
  ...
  const fmt = transactions.map(t => ({
    ...t, 
    amountFormatted: t.amount >= 0 ? `+$${Math.abs(t.amount).toFixed(2)}` : `-$${Math.abs(t.amount).toFixed(2)}`, 
...
app.get('/transfer', requireLogin, async (req, res) => {
  res.render('transfer', { 
    user: req.session.user, 
    accounts: await sqlAll(`SELECT * FROM accounts WHERE userId = ?`, [req.session.user.id]), 
    success: null, error: null, successAmount: null 
  });
});
...
app.post('/transfer', requireLogin, async (req, res) => {
  ...
  if (!fromAcc || isNaN(amt) || amt <= 0 || amt > fromAcc.balance) {
    return res.render('transfer', { 
      user: req.session.user, 
      accounts: await sqlAll(`SELECT * FROM accounts WHERE userId = ?`, [uid]), 
      success: null, error: 'Invalid or insufficient funds.', successAmount: null 
    });
  }
  ...
  if (transferType === 'internal') {
    if (!toAcc) {
      return res.render('transfer', { 
        user: req.session.user, 
        accounts: await sqlAll(`SELECT * FROM accounts WHERE userId = ?`, [uid]), 
        success: null, error: 'Select destination.', successAmount: null 
      });
    }
    ...
    await sqlRun(`INSERT INTO transactions (userId, accountId, type, amount, description, date) VALUES (?,?,?,?,?,?)`, 
      [uid, fromAcc.id, 'Transfer Out', -amt, desc, new Date().toISOString()]);
    await sqlRun(`INSERT INTO transactions (userId, accountId, type, amount, description, date) VALUES (?,?,?,?,?,?)`, 
      [uid, toAcc.id, 'Transfer In', amt, `From ${fromAcc.name}`, new Date().toISOString()]);
  } else if (transferType === 'domestic') {
    fee = 25; 
    label = 'Domestic Wire'; 
    toName = recipientName; 
    desc = `Wire to ${recipientName}`;
    await sqlRun(`UPDATE accounts SET balance = balance - ? WHERE id = ?`, [amt+fee, fromAcc.id]);
    await sqlRun(`INSERT INTO transactions (userId, accountId, type, amount, description, date) VALUES (?,?,?,?,?,?)`, 
      [uid, fromAcc.id, 'Wire Out', -amt, desc, new Date().toISOString()]);
    await sqlRun(`INSERT INTO transactions (userId, accountId, type, amount, description, date) VALUES (?,?,?,?,?,?)`, 
      [uid, fromAcc.id, 'Fee', -fee, 'Wire Fee', new Date().toISOString()]);
  } else {
    fee = 45; 
    label = 'International Wire'; 
    toName = `${recipientName} (${country||'Intl'})`; 
    desc = `Intl Wire to ${recipientName}`;
    await sqlRun(`UPDATE accounts SET balance = balance - ? WHERE id = ?`, [amt+fee, fromAcc.id]);
    await sqlRun(`INSERT INTO transactions (userId, accountId, type, amount, description, date) VALUES (?,?,?,?,?,?)`, 
      [uid, fromAcc.id, 'Wire Out', -amt, desc, new Date().toISOString()]);
    await sqlRun(`INSERT INTO transactions (userId, accountId, type, amount, description, date) VALUES (?,?,?,?,?,?)`, 
      [uid, fromAcc.id, 'Fee', -fee, 'Intl Wire Fee', new Date().toISOString()]);
  }
  
  await sqlRun(`INSERT INTO transfers (userId, fromAccount, toAccount, amount, date, status) VALUES (?,?,?,?,?,?)`, 
    [uid, fromAcc.id, toAcc?.id||null, amt, new Date().toISOString(), 'completed']);
  ...
  res.render('transfer-confirm', { 
    ...
    toAccount: toName, 
    transferType: label, 
    arrivalDays: fee ? '1-3 Days' : 'Instant', 
    memo: memo||null, 
...
app.get('/billpay', requireLogin, async (req, res) => 
  res.render('billpay', { 
    ...
    successAmount: req.query.amount 
  })
);
...
app.post('/billpay', requireLogin, async (req, res) => {
  ...
  if (!fromAcc || isNaN(amt) || amt <= 0 || amt > fromAcc.balance) {
    return res.redirect('/billpay?error=Invalid+payment');
  }
  ...
  await sqlRun(`INSERT INTO transactions (userId, accountId, type, amount, description, date) VALUES (?,?,?,?,?,?)`, 
    [uid, fromAcc.id, 'Bill Pay', -amt, `Bill to ${payeeName}`, new Date().toISOString()]);
...
app.get('/billpay/history', requireLogin, async (req, res) => 
  res.render('history', { 
    user: req.session.user, 
    transactions: await sqlAll(`SELECT * FROM billpay_transactions WHERE userId = ? ORDER BY date DESC`, [req.session.user.id]) 
  })
);
...
app.get('/admin', requireAdmin, async (req, res) => 
  res.render('admin', { 
    user: req.session.user, 
    users: await sqlAll(`SELECT id, username, email, isAdmin, isLocked, created_at FROM users ORDER BY id`) 
  })
);
...
app.get('/admin/users', requireAdmin, async (req, res) => { 
  ...
  res.render('admin_users', { 
    user: req.session.user, 
    users: await sqlAll(`SELECT id, username, email, isAdmin, isLocked, created_at FROM users WHERE username LIKE ? OR email LIKE ? ORDER BY id`, [`%${q}%`,`%${q}%`]), 
    query: q 
  }); 
});
...
app.get('/admin/users/:id', requireAdmin, async (req, res) => {
  ...
  res.render('admin_user', { 
    user: req.session.user, 
    viewedUser: await sqlGet(`SELECT * FROM users WHERE id = ?`, [uid]), 
    accounts: await sqlAll(`SELECT * FROM accounts WHERE userId = ?`, [uid]), 
    bills: await sqlAll(`SELECT * FROM billpay_transactions WHERE userId = ? ORDER BY date DESC`, [uid]), 
    trans: await sqlAll(`SELECT t.*, a.name AS accountName FROM transactions t LEFT JOIN accounts a ON t.accountId = a.id WHERE t.userId = ? ORDER BY t.date DESC`, [uid]), 
    transfers: await sqlAll(`SELECT * FROM transfers WHERE userId = ? ORDER BY date DESC`, [uid]) 
  });
});

app.post('/admin/users/:id/toggleAdmin', requireAdmin, async (req, res) => { 
  const u = await sqlGet(`SELECT isAdmin FROM users WHERE id = ?`, [req.params.id]); 
  await sqlRun(`UPDATE users SET isAdmin = ? WHERE id = ?`, [u.isAdmin?0:1, req.params.id]); 
  res.redirect(`/admin/users/${req.params.id}`); 
});

app.post('/admin/users/:id/lock', requireAdmin, async (req, res) => { 
  const lock = new Date(Date.now() + (parseInt(req.body.minutes)||60)*60000).toISOString(); 
  await sqlRun(`UPDATE users SET isLocked = 1, lockUntil = ? WHERE id = ?`, [lock, req.params.id]); 
  res.redirect(`/admin/users/${req.params.id}`); 
});

app.post('/admin/users/:id/unlock', requireAdmin, async (req, res) => { 
  await sqlRun(`UPDATE users SET isLocked = 0, lockUntil = NULL WHERE id = ?`, [req.params.id]); 
  res.redirect(`/admin/users/${req.params.id}`); 
});
