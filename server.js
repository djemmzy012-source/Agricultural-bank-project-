...
app.post('/register', async (req, res) => {
  const { 
    ...
    confirmPassword, accountType, address, city, state, zip 
  } = req.body;
...
app.post('/forgot-password', async (req, res) => {
  ...
  // In production: send real reset email via nodemailer/SendGrid
  // For demo: show current password (NOT secure - for development only)
  res.render('forgot-password', {
    error: null,
    success: 'Password reset link sent to your email!',
    info: 'Demo mode: Your current password is: Andre44225'
  });
});
    ...
    if (accountId) {
      ...
      transactions = await sqlAll(
        `SELECT t.*, a.name AS accountName, a.number AS accountNumber
         FROM transactions t 
         LEFT JOIN accounts a ON t.accountId = a.id
         WHERE t.userId = ? AND t.accountId = ? 
         ORDER BY t.date DESC`,
        [uid, accountId]
      );
...
app.post('/transfer', requireLogin, async (req, res) => {
  try {
    const { 
      transferType, fromAccount, toAccount, amount, memo, 
      recipientName, bankName, routingNumber, accountNumber, swiftCode, country, purpose 
    } = req.body;
    ...
    switch (transferType) {
      ...
      case 'domestic':
        ...
        toAccountName = recipientName || 'External Account';
        ...
        await sqlRun(
          `INSERT INTO transactions (userId, accountId, type, amount, description, date) 
           VALUES (?, ?, ?, ?, ?, ?)`,
          [uid, fromAcc.id, 'Fee', -fee, 'Domestic Wire Fee', new Date().toISOString()]
        );
        break;
      ...
      case 'international':
        ...
        toAccountName = `${recipientName} (${country || 'Intl'})`;
        ...
        await sqlRun(
          `INSERT INTO transactions (userId, accountId, type, amount, description, date) 
           VALUES (?, ?, ?, ?, ?, ?)`,
          [uid, fromAcc.id, 'Fee', -fee, 'International Wire Fee', new Date().toISOString()]
        );
        break;

      default:
        arrivalDays = '1-3 Business Days';
        transferLabel = 'External Transfer';
    ...
    // Record transfer
    await sqlRun(
      `INSERT INTO transfers (userId, fromAccount, toAccount, amount, date, status) 
       VALUES (?, ?, ?, ?, ?, ?)`,
...
app.post('/billpay', requireLogin, async (req, res) => {
  try {
    ...
    if (!fromAcc || isNaN(amt) || amt <= 0) {
      return res.redirect('/billpay?error=' + encodeURIComponent('Invalid payment details.'));
    }
    if (amt > fromAcc.balance) {
    ...
    await sqlRun(
      `INSERT INTO billpay_transactions (userId, fromAccount, amount, date, payeeName) 
       VALUES (?, ?, ?, ?, ?)`,
...
app.get('/billpay/history', requireLogin, async (req, res) => {
  try {
    const uid = req.session.user.id;
    const transactions = await sqlAll(
      `SELECT * FROM billpay_transactions WHERE userId = ? ORDER BY date DESC`, 
      [uid]
    );
...
app.get('/billpay/history/export', requireLogin, async (req, res) => {
  try {
    ...
    const csv = [header, ...rows.map(r => 
      [r.id, r.userId, r.fromAccount, r.amount, r.date, r.payeeName].join(',')
    )].join('\n');
...
app.post('/profile', requireLogin, async (req, res) => {
  try {
    ...
    await sqlRun(
      `UPDATE users SET email = ?, address = ?, city = ?, state = ?, zip = ? WHERE id = ?`,
...
app.get('/admin', requireAdmin, async (req, res) => {
  try {
    const users = await sqlAll(
      `SELECT id, username, email, isAdmin, isLocked, created_at FROM users ORDER BY id`
    );
...
app.post('/admin/users/:id/lock', requireAdmin, async (req, res) => {
  try {
    ...
    await sqlRun(
      `UPDATE users SET isLocked = 1, lockUntil = ? WHERE id = ?`, 
      [lockUntil, uid]
    );
    ...
    res.status(500).send('Failed to lock user');
  }
});
...
app.post('/admin/users/:id/unlock', requireAdmin, async (req, res) => {
  try {
    ...
    await sqlRun(
      `UPDATE users SET isLocked = 0, lockUntil = NULL WHERE id = ?`, 
      [uid]
    );
...
app.get('/admin/export/users', requireAdmin, async (req, res) => {
  try {
    ...
    const csv = [header, ...rows.map(r => 
      [r.id, r.username, r.email, r.isAdmin, r.isLocked, r.created_at].join(',')
    )].join('\n');
...
app.get('/admin/users/:id/export/billpay', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('Bill pay export error:', error);
    res.status(500).send('Export failed');
  }
});
...
app.get('/admin/users/:id/export/transactions', requireAdmin, async (req, res) => {
  try {
    ...
    const csv = [header, ...rows.map(r => 
      [r.id, r.accountId, r.type, r.amount, r.description, r.date, r.accountName].join(',')
    )].join('\n');
...
<html><head><title>✅ Demo Data Seeded</title>
...
<style>
  ...
  <div class="box">
    ...
    <p>Added <strong>${count?.c || 0} transactions</strong> to Andrew's accounts.</p>
    <div class="stats">
      <div><strong>Primary Checking (****4521):</strong> 14 txns → <span class="total">$42,420.50</span></div>
      <div><strong>Savings Account (****8934):</strong> 14 txns → <span class="total">$325,890.25</span></div>
      <div><strong>Business Operating (****2156):</strong> 14 txns → <span class="total">$595,697.25</span></div>
